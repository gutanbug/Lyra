...
React + TypeScript + Electron 기반 데스크톱 애플리케이션.
...
```shell
pnpm electron:dev   # 개발 모드 (React HMR + Electron)
pnpm electron:start  # 프로덕션 빌드 후 Electron 실행
pnpm electron:build  # 배포용 패키지 생성 (release/ 폴더)
```

- **electron:dev**: React 개발 서버와 Electron을 동시에 실행합니다. HMR이 적용됩니다.
- **electron:start**: React 빌드 후 Electron 앱을 실행합니다.
- **electron:build**: Mac(dmg, zip), Windows(nsis, portable), Linux(AppImage, deb)용 설치 파일을 생성합니다.
